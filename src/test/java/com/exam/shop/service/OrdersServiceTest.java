...
class OrdersServiceTest {
    ...
    void orderCancelTest() {
        ...
        Optional<Book> findBook = itemService.findByBook("book1");

        if (findMember.isPresent() && findBook.isPresent()) {
            Long orderId = ordersService.ordersCreate(findMember.get().getId(), findBook.get().getId(), 10);
            entityManager.flush();
            entityManager.clear();
        ...
        List<OrdersDto> ordersDtos = ordersService.searchOrdersWithItems(condition);
        assertThat(ordersDtos).extracting("orderStatus").containsExactly(OrderStatus.CANCEL);
    }
}